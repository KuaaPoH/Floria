...
*Ngày cập nhật: 25 tháng 06, 2026*
...
**Bước 1: Sửa lỗi môi trường khởi chạy Web (Ưu tiên cao nhất cho sáng mai)**
- **Hiện trạng lỗi:** Chạy App Web trên cổng 5000 đôi lúc gây ra tình trạng thời gian tải rất lâu hoặc kẹt ở màn hình trắng.
- **Giải pháp tạm thời đã thêm:** Thêm CSS Loading Spinner vào `index.html` để tránh màn hình trắng tinh, và thêm chốt chặn `Firebase.apps.isEmpty` tại `main.dart` để tránh lỗi xung đột khi Hot Restart.
- **Công việc ngày mai:** Soi kỹ Console (F12) của trình duyệt Chrome để tìm cội nguồn gây ra lỗi trắng trang, đảm bảo luồng Đăng nhập Google mượt mà không độ trễ.

**Bước 2: Nối cáp thông luồng AI thực tế (Ưu tiên cao nhất)**
...
**Bước 3: Nối Cơ Sở Dữ Liệu Thực (Firestore)**
- Gửi `JWT Token` từ Flutter app (sau khi Auth thành công) lên Spring Boot.
- Kiểm tra tính năng lưu lịch sử bệnh (`ScanHistory`) vào Firestore thành công.

**Bước 3: Đánh thức Trợ lý Ảo (AI Chatbot - Gemini)**
- Tích hợp Gemini API vào thư mục `lib/features/chatbot/` trên Flutter/Web để trả lời tư vấn cây trồng.

**Bước 4: Hoàn thiện Cửa hàng (Shop & Payment)**
